...
# ! changelog : non-hardcoded implementation, dynamic mapping
...
XLSX_FILE_PATH = "output/consolidate_parser_output_new.xlsx"
...
def feature_parser(root, ns, feature_type_mapping, reliability_mapping):
    ...
    for party in distinct_parties:
        ...
        for feature in features:
            ...
            if feature_type == "Location":
                value = feature_version.find(".//ns:VersionLocation", ns).attrib.get(
                    "LocationID", ""
                )
            elif feature_type == "Birthdate":
                date_period_element = feature_version.find(".//ns:DatePeriod", ns)
                if date_period_element is not None:
                    start = date_period_element.find(".//ns:Start", ns)
                    end = date_period_element.find(".//ns:End", ns)
...
def name_parser(
    ...
    """Parses names from the XML root and returns field names and data rows."""
    ...
    for party in root.findall(".//ns:DistinctParty", ns):
        ...
        for profile in party.findall(".//ns:Profile", ns):
            ...
            designation = party_subtype_values.get(party_subtype_id, "Unknown")
...
def main():
    if download_xml(XML_URL, XML_FILE_PATH):
        ...
        feature_fieldnames, feature_data_rows = feature_parser(
            root, NAMESPACE, feature_type_mapping, reliability_mapping
        )
        # Parse IDs
